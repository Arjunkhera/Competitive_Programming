...
void solve(vector<string> &board,int n,int &answer,int row){
  if(row == n){ answer += 1; return; }

  for(int i = 0;i < n;i++){
    board[row][i] = 'Q';
    if(isSafe(board,n,row,i)) solve(board,n,answer,row+1);
    // backtrack
    board[row][i] = '.';
  }
}

void solveoptimized(vector<string> &board,int n,int &answer,int row,vector<bool> &col,vector<bool> &ld,vector<bool> &rd){
